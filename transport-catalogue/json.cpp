...
void Node::SetValue(Node::Value value) {
    if (std::holds_alternative<bool>(value)) {
        *this = std::get<bool>(value);
    }
    if (std::holds_alternative<double>(value)) {
        *this = std::get<double>(value);
    }
    if (std::holds_alternative<std::nullptr_t>(value)) {
        *this = std::get<std::nullptr_t>(value);
    }
    if (std::holds_alternative<int>(value)) {
        *this = std::get<int>(value);
    }
    if (std::holds_alternative<json::Array>(value)) {
        *this = std::get<json::Array>(value);
    }
    if (std::holds_alternative<std::string>(value)) {
        *this = std::get<std::string>(value);
    }
    if (std::holds_alternative<json::Dict>(value)) {
        *this = std::get<json::Dict>(value);
    }
}
